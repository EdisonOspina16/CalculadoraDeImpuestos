...
sys.path.append("src")
from logic.input_validator import MIN_PROPERTY_VALUE, MAX_PROPERTY_VALUE, MIN_AGE_LIMIT, MIN_INTEREST_RATE, \
    MAX_INTEREST_RATE
...
def get_input(prompt: str, expected_type: type = str, valid_values: list = None):
    ...
    while True:
        ...
        if expected_type == float:
            ...
            if ',' in user_input:
                print(
                    f"Error: Invalid characters found in input: '{user_input}'. Please use a decimal point instead of a comma.")
                continue
            ...
            try:
                ...
                if "interest rate" in prompt.lower():
                    if value <= MIN_INTEREST_RATE or value > MAX_INTEREST_RATE:
                        print(
                            f"Error: Interest rate must be between {MIN_INTEREST_RATE} and {MAX_INTEREST_RATE}. You entered: {value}.")
                        continue
            ...
            try:
                ...
                if "property value" in prompt.lower():
                    if value <= 0:
                        ...
                        print(
                            f"Error: Property value must be between {MIN_PROPERTY_VALUE:,} and {MAX_PROPERTY_VALUE:,}. You entered: {value}.")
                        continue
